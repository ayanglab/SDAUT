...
def main(json_path):
    ...
    for idx, test_data in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            # attention_map = model.get_attn_map()
        ...
        with open(os.path.join(save_dir, 'results.csv'), 'a') as cf:
            writer = csv.writer(cf)
            writer.writerow(['SDAUT', dataset_opt['mask'],
...
def define_model(args):
    ...
    model = net(img_size=args['netG']['img_size'],
               patch_size=args['netG']['patch_size'],
               in_chans=args['netG']['in_chans'],
               embed_dims=args['netG']['embed_dims'],
               types=args['netG']['type'],
               depths=args['netG']['depths'],
               n_heads=args['netG']['num_heads'],
               n_groups=args['netG']['n_group'],
               window_size=args['netG']['window_size'],
               mlp_ratio=args['netG']['mlp_ratio'],
               img_range=args['netG']['img_range'],
               use_pe=args['netG']['use_pe'],
               dwc_pe=args['netG']['dwc_pe'],
               no_off=args['netG']['no_off'],
               fixed_pe=args['netG']['fixed_pe'], )
