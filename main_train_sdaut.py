...
def main(json_path='options/SDAUT/train_sdaut_CC_G1D30.json'):
    '''
    # ----------------------------------------
    # Step--1 (prepare opt)
    # ----------------------------------------
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                for idx, test_data in enumerate(test_loader):
                    ...
                    if idx < 10:
                        util.mkdir(img_dir)
                        save_img_path = os.path.join(img_dir, 'ZF_{:05d}.png'.format(current_step))
                        util.imsave(L_img, save_img_path)
                        save_img_path = os.path.join(img_dir, 'Recon_{:05d}.png'.format(current_step))
                        util.imsave(E_img, save_img_path)
                        save_img_path = os.path.join(img_dir, 'GT_{:05d}.png'.format(current_step))
                        util.imsave(H_img, save_img_path)

                    save_img_tmp_path_L = os.path.join(img_dir_tmp_L, '{}.png'.format(img_name))
                    util.imsave(L_img, save_img_tmp_path_L)
                    save_img_tmp_path_E = os.path.join(img_dir_tmp_E, '{}.png'.format(img_name))
                    util.imsave(E_img, save_img_tmp_path_E)
                    save_img_tmp_path_H = os.path.join(img_dir_tmp_H, '{}.png'.format(img_name))
                    util.imsave(H_img, save_img_tmp_path_H)
                ...
                logger.info(
                    '<epoch:{:3d}, iter:{:8,d}, FID : {:<.4f}dB\n'.format(epoch, current_step, fid))
...
if __name__ == '__main__':
    ...
    main('options/SDAUT/train_sdaut_CC_G1D30.json')
